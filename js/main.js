(function() {
  ...
  function changeChord() {
    ...
    var chord_type = document.getElementById("chord-type").value;
    var chord_ext = document.getElementById("chord-ext").value;
    var minSecond = notes[(notes.indexOf(root) + 1) % 12];
    var majSecond = notes[(notes.indexOf(root) + 2) % 12];
    var minThird = notes[(notes.indexOf(root) + 3) % 12];
    var majThird = notes[(notes.indexOf(root) + 4) % 12];
    var forth = notes[(notes.indexOf(root) + 5) % 12];
    var tritone = notes[(notes.indexOf(root) + 6) % 12];
    var fifth = notes[(notes.indexOf(root) + 7) % 12];
    var minSixth = notes[(notes.indexOf(root) + 8) % 12];
    var majSixth = notes[(notes.indexOf(root) + 9) % 12];
    var minSeventh = notes[(notes.indexOf(root) + 10) % 12];
    var majSeventh = notes[(notes.indexOf(root) + 11) % 12];

    var orderNotes = [root, minSecond, majSecond, minThird, majThird, forth, tritone,fifth, minSixth,majSixth,minSeventh,majSeventh]
